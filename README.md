...
For Codex, `relay account login` creates a missing profile directory with user-only permissions, then runs the native `codex login` flow with both Codex state directories pinned to that absolute path. Relay forces ChatGPT authentication, forces the Codex CLI's file-backed credential store inside that profile, and removes shell-level `OPENAI_API_KEY` and `CODEX_ACCESS_TOKEN` overrides. The Codex CLI owns its credential file; Relay never reads or copies it.

After native login, Relay asks the official [`codex app-server` `account/read` endpoint](https://github.com/openai/codex/blob/main/codex-rs/app-server/README.md#1-check-auth-state) for the selected profile's ChatGPT account. Relay normalizes and hashes the reported email, stores only that opaque fingerprint, and checks it again before profile-scoped work. Missing authentication, an unavailable identity, or a profile authenticated as another account fails closed before Relay creates provider work.

Identity checks are short-lived and serialized inside one Relay process. Plugins are disabled for the probe, and all accounts reuse one small probe-only Codex SQLite directory beside Relay's database—there is no per-account database or resident daemon. Credentials remain in each selected `CODEX_HOME`; Relay stores neither the reported email nor provider tokens. An older Codex CLI without the app-server account surface can still expose default-profile cloud commands, but Relay will not advertise isolated multi-account operation. Profiles registered before this identity binding was added require one new `relay account login <id>` before use.
...
Each provider profile is a native identity boundary with its own settings, credentials, and session history. Relay does not copy or swap credentials and does not put tokens or account emails in SQLite.
