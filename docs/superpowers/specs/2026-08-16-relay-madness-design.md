...
A ProviderAccount holds a provider, label, local profile reference, status, concurrency capacity, and optional default flag. `CODEX_HOME`, `CODEX_SQLITE_HOME`, and `CLAUDE_CONFIG_DIR` are references only: Relay never stores provider credentials. Codex profiles use native, file-backed ChatGPT login under their own state directory. Relay reads the selected profile's non-secret email through the official `codex app-server` account surface, persists only its opaque digest, verifies it before execution, and fails closed if authentication disappears or resolves to another identity. The probe disables plugins, reuses one probe-only Codex SQLite directory for the whole Relay installation, is serialized within one Relay process, and leaves no background process or per-account database. Claude profiles use the native login associated with their own configuration directory; Relay likewise persists only an opaque identity fingerprint, verifies it before execution, and fails closed if authentication disappears or resolves to another identity.
